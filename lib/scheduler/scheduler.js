...
dtmlXMLLoaderObject.prototype.loadXML = function(a, b, c, d) {
	this.rSeed && (a += (a.indexOf("?") != -1 ? "&" : "?") + "a_tmn_rSeed=" + (new Date).valueOf());
	this.filePath = a;
...
(function() {
	...
	function c(a) {
		if (!c.cover) c.cover = document.createElement("DIV"), c.cover.onkeydown = b, c.cover.className = "tmn_modal_cover", document.body.appendChild(c.cover);
	...
	dhtmlx.modalbox.hide = function(a) {
		for (; a && a.getAttribute && !a.getAttribute("tmnbox");) a = a.parentNode;
...
dataProcessor.prototype = {
	...
	serialize: function(a, b) {
		...
		if (typeof b != "undefined") return this.serialize_one(a, "");
			...
			dhtmlx.security_key && c.push("tmn_security=" +
				dhtmlx.security_key);
			return c.join("&")
		}
	},
		...
		if (a) {
			...
			var c = new dtmlXMLLoaderObject(this.afterUpdate, this, !0),
				d = this.serverProcessor + (this._user ? getUrlSymbol(this.serverProcessor) +
					["tmn_user=" + this._user, "tmn_version=" + this.obj.getUserData(0, "version")].join("&") : "");
	...
	loadUpdate: function() {
		var a = this,
			...
			c = this.serverProcessor + getUrlSymbol(this.serverProcessor) + ["tmn_user=" + this._user, "tmn_version=" + b].join("&"),
...
if (window.dhtmlXGridObject) dhtmlXGridObject.prototype._init_point_connector = dhtmlXGridObject.prototype._init_point, dhtmlXGridObject.prototype._init_point = function() {
	var a = function(a) {
		...
		return a + (a.indexOf("?") != -1 ? "&" : "?") + "connector=true" + (this.hdr.rows.length > 0 ? "&tmn_no_header=1" : "")
	}, b = function(b) {
			return a.call(this, b) + (this._connector_sorting || "") + (this._connector_filter || "")
		}, c = function(a, c, d) {
			this._connector_sorting = "&tmn_sort[" + c + "]=" + d;
			return b.call(this,
				a)
		}, d = function(a, c, d) {
			for (var h = 0; h < c.length; h++) c[h] = "tmn_filter[" + c[h] + "]=" + encodeURIComponent(d[h]);
	...
	if (!this._colls_loaded && this.cellType) {
		...
		d.length && (arguments[0] += (arguments[0].indexOf("?") != -1 ? "&" :
			"?") + "connector=true&tmn_colls=" + d.join(","))
	}
...
scheduler.set_sizes = function() {
	...
	this.set_xy(this._els.tmn_cal_navline[0], a, this.xy.nav_height, 0, 0);
	this.set_xy(this._els.tmn_cal_header[0], a - c, this.xy.scale_height, d, this.xy.nav_height + (this._quirks ? -1 : 1));
	var e = this._els.tmn_cal_navline[0].offsetHeight;
	if (e > 0) this.xy.nav_height = e;
	var f = this.xy.scale_height +
		this.xy.nav_height + (this._quirks ? -2 : 0);
	this.set_xy(this._els.tmn_cal_data[0], a, b - (f + 2), 0, f + 2)
};
...
scheduler._on_dbl_click = function(a, b) {
	...
	if (!this.config.readonly && b.className) {
		...
		switch (c) {
			case "tmn_scale_holder":
			case "tmn_scale_holder_now":
			case "tmn_month_body":
			case "tmn_wa_day_data":
			case "tmn_marked_timespan":
				if (!scheduler.config.dblclick_create) break;
				this.addEventNow(this.getActionData(a).date, null, a);
				break;
			case "tmn_cal_event":
			case "tmn_wa_ev_body":
			case "tmn_agenda_line":
			case "tmn_grid_event":
			case "tmn_cal_event_line":
			
				
			case "tmn_cal_event_clear":
				var d =
...
scheduler._mouse_coords = function(a) {
	...
	b.y -= getAbsoluteTop(this._obj) + this.xy.nav_height + (this._dy_shift || 0) + this.xy.scale_height - this._els.tmn_cal_data[0].scrollTop;
	b.ev = a;
		...
		if (!this._drag_pos || b.force_redraw || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
			...
			if (this._drag_mode == "move")
				if (c = this._min_date.valueOf() + (b.y * this.config.time_step + b.x * 1440 - (scheduler._move_pos_shift || 0)) *
					...
					} else if (d = this.date.date_part(new Date(h.end_date)).valueOf() + b.y * this.config.time_step * 6E4, this._els.tmn_cal_data[0].style.cursor = "s-resize", this._mode == "week" || this._mode == "day") d = this._correct_shift(d);
			...
			if (this._table_view || l.getDate() == n.getDate() && l.getHours() < this.config.last_hour || scheduler._allow_dnd)
				if (h.start_date = n, h.end_date = new Date(d), this.config.update_render) {
					var m = scheduler._els.tmn_cal_data[0].scrollTop;
					this.update_view();
					scheduler._els.tmn_cal_data[0].scrollTop = m
				} else this.updateEvent(this._drag_id);
			this._table_view && this.for_rendered(this._drag_id, function(a) {
				a.className += " tmn_in_move"
			})
		}
	...
	if (a.button != 2 && !this.config.readonly && !this._drag_mode) {
		...
		switch (c) {
			...
			case "tmn_wa_ev_body":
				...
				break;
			case "tmn_event_resize":
				this._drag_mode = "resize";
				break;
			case "tmn_scale_holder":
			case "tmn_scale_holder_now":
			case "tmn_month_body":
			case "tmn_matrix_cell":
			case "tmn_marked_timespan":
...
scheduler._on_mouse_up = function(a) {
	if (!a || !(a.button == 2 && scheduler.config.touch)) {
		if (this._drag_mode && this._drag_id) {
			this._els.tmn_cal_data[0].style.cursor = "default";
			var b = this.getEvent(this._drag_id);
			if (this._drag_event._tmn_changed || !this._drag_event.start_date || b.start_date.valueOf() != this._drag_event.start_date.valueOf() || b.end_date.valueOf() != this._drag_event.end_date.valueOf()) {
				...
				} else c ? this.deleteEvent(b.id, !0) : (this._drag_event._tmn_changed = !1, scheduler._lame_copy(b, this._drag_event), this.updateEvent(b.id))
			}
			this._drag_pos &&
...
scheduler.updateView = function(a, b) {
	var a = a || this._date,
		b = b || this._mode,
		c = "tmn_cal_data";
	this._mode ? this._obj.className = this._obj.className.replace("tmn_scheduler_" + this._mode, "tmn_scheduler_" + b) : this._obj.className += " tmn_scheduler_" + b;
...
scheduler._reset_scale = function() {
	if (this.templates[this._mode + "_date"]) {
		var a = this._els.tmn_cal_header[0],
			b = this._els.tmn_cal_data[0],
			c = this.config;
		a.innerHTML = "";
		b.scrollTop = 0;
		b.innerHTML = "";
		var d = (c.readonly || !c.drag_resize ? " tmn_resize_denied" : "") + (c.readonly || !c.drag_move ? " tmn_move_denied" : "");
		if (d) b.className = "tmn_cal_data" + d;
		this._scales = {};
		this._cols = [];
		this._colsS = {
			height: 0
		...
		this._min_date = g;
		this._els.tmn_cal_date[0].innerHTML = this.templates[this._mode + "_date"](h, j, this._mode);
		...
		for (var m = l - this._ignores_detected, o = 0; o < l; o++) {
			...
			if (!this._table_view) {
				var p = document.createElement("DIV"),
					q = "tmn_scale_holder";
				g.valueOf() == i.valueOf() && (q = "tmn_scale_holder_now");
		...
		if (this._table_view) this._reset_month_scale(b, h, k);
			...
			var s = this._els.tmn_cal_navline[0],
				u = s.offsetHeight + this._els.tmn_cal_header[0].offsetHeight + 1,
...
scheduler._reset_month_scale = function(a, b, c) {
	...
	for (var n = "<table cellpadding='0' cellspacing='0'>",
		...
		for (var o = 0; o < 7; o++) {
			n += "<td";
			var p = "";
			c < b ? p = "tmn_before" : c >= d ? p = "tmn_after" : c.valueOf() == e.valueOf() && (p = "tmn_now");
			n += " class='" + p + " " + this.templates.month_date_class(c, e) + "' >";
			var q = "tmn_month_body",
				r = "tmn_month_head";
			o === 0 && this.config.left_border && (q += " tmn_month_body_border", r += " tmn_month_head_border");
...
scheduler.locale = {
	...
	labels: {
		...
		previous_tab: "Previous Month",
		next_tab: "Next Month",
		tmn_cal_today_button: "Today",
		day_tab: "Day",
		week_tab: "Week",
		month_tab: "Month",
		new_event: "New event",
...
scheduler.config = {
	...
	buttons_left: ["tmn_save_btn", "tmn_cancel_btn"],
	buttons_right: ["tmn_delete_btn"],
	lightbox: {
		sections: [{
			name: "description",
			height: 200,
...
scheduler.init_templates = function() {
	...
	a.tmn_save_btn = a.icon_save;
	a.tmn_cancel_btn = a.icon_cancel;
	a.tmn_delete_btn = a.icon_delete;
...
scheduler.render_view_data = function(a, b) {
	...
	if (this.config.multi_day && !this._table_view) {
		...
		this._rendered_location = this._els.tmn_multi_day[0];
		this._table_view = !0;
		this.render_data(f, b);
		this._table_view = !1;
		this._rendered_location = this._els.tmn_cal_data[0];
		this._table_view = !1;
		this.render_data(e, b)
	} else this._rendered_location = this._els.tmn_cal_data[0], this.render_data(a, b)
};
...
scheduler._pre_render_events = function(a, b) {
	...
	if (this._table_view)
		if (b) this._colsS.heights = d;
			...
			} else if (!a.length && this._els.tmn_multi_day[0].style.visibility == "visible" && (e[0] = -1), a.length || e[0] == -1) {
				var o = g.parentNode.childNodes,
					p = (e[0] + 1) * c + 1,
					q = p,
					r = p + "px";
				this.config.multi_day_height_limit && (q = Math.min(p, this.config.multi_day_height_limit), r = q + "px");
				f.style.top = this._els.tmn_cal_navline[0].offsetHeight + this._els.tmn_cal_header[0].offsetHeight + q + "px";
				f.style.height = this._obj.offsetHeight -
					parseInt(f.style.top, 10) - (this.xy.margin_top || 0) + "px";
				var s = this._els.tmn_multi_day[0];
				s.style.height = r;
				s.style.visibility = e[0] == -1 ? "hidden" : "visible";
				var u = this._els.tmn_multi_day[1];
				u.style.height = r;
				u.style.visibility = e[0] == -1 ? "hidden" : "visible";
				u.className = e[0] ? "tmn_multi_day_icon" : "tmn_multi_day_icon_small";
	...
	if (!(a._sday < 0)) {
		...
		if (d) {
			...
			if (this._edit_id ==
				...
				p.className = "tmn_cal_editor";
				var q = document.createElement("DIV");
				this.set_xy(q, j - 6, i - 26);
				q.style.cssText += ";margin:2px 2px 2px 2px;overflow:hidden;";
				p.appendChild(q);
				this._els.tmn_cal_data[0].appendChild(p);
				this._rendered.push(p);
				q.innerHTML = "<textarea class='tmn_cal_editor'>" + a.text + "</textarea>";
				...
				this._els.tmn_cal_data[0].scrollLeft = 0
			}
			if (this.xy.menu_width !== 0 && this._select_id == a.id) {
				if (this.config.cascade_event_display && this._drag_mode) p.style.zIndex = 1;
				for (var r =
					this.config["icons_" + (this._edit_id == a.id ? "edit" : "select")], s = "", u = a.color ? "background-color: " + a.color + ";" : "", t = a.textColor ? "color: " + a.textColor + ";" : "", v = 0; v < r.length; v++) s += "<div class='tmn_menu_icon " + r[v] + "' style='" + u + "" + t + "' title='" + this.locale.labels[r[v]] + "'></div>";
				var x = this._render_v_bar(a.id, l - b + 1, k, b, r.length * 20 + 26 - 2, "", "<div style='" + u + "" + t + "' class='tmn_menu_head'></div>", s, !0);
				x.style.left = l - b + 1;
				this._els.tmn_cal_data[0].appendChild(x);
...
scheduler._render_v_bar = function(a, b, c, d, e, f, g, h, k) {
	var i = document.createElement("DIV"),
		j = this.getEvent(a),
		l = k ? "tmn_cal_event tmn_cal_select_menu" : "tmn_cal_event",
	...
	if (k || !scheduler.renderEvent(q, j)) {
		var q = i.firstChild,
			r = '<div class="tmn_event_move tmn_header" style=" width:' + (d - 6) + "px;" + m + '" >&nbsp;</div>';
		r += '<div class="tmn_event_move tmn_title" style="' + m + "" + o + '">' + g + "</div>";
		r += '<div class="tmn_body" style=" width:' + (d - (this._quirks ? 4 : 14)) + "px; height:" + (e - (this._quirks ? 20 : 30) + 1) + "px;" + m + "" + o + '">' + h + "</div>";
		var s = "tmn_event_resize tmn_footer";
		k && (s = "tmn_resize_denied " + s);
...
scheduler.locate_holder = function(a) {
	return this._mode == "day" ? this._els.tmn_cal_data[0].firstChild : this._els.tmn_cal_data[0].childNodes[a]
};
...
scheduler.render_event_bar = function(a) {
	...
	if (f) {
		var g = document.createElement("DIV"),
			h = "tmn_cal_event_clear";
		a._timed || (h = "tmn_cal_event_line", a.hasOwnProperty("_first_chunk") && a._first_chunk && (h += " tmn_cal_event_line_start"), a.hasOwnProperty("_last_chunk") && a._last_chunk && (h += " tmn_cal_event_line_end"));
...
scheduler.showEvent = function(a, b) {
	...
	if (c && (!this.checkEvent("onBeforeEventDisplay") || this.callEvent("onBeforeEventDisplay", [c, b]))) {
		...
		if (scheduler.matrix && scheduler.matrix[b]) scheduler._els.tmn_cal_data[0].scrollTop = getAbsoluteTop(scheduler.getRenderedEvent(c.id)) - getAbsoluteTop(scheduler._els.tmn_cal_data[0]) - 20;
...
scheduler.json.parse = function(a) {
	...
	if (a.tmn_security) dhtmlx.security_key = a.tmn_security;
...
scheduler._magic_parser = function(a) {
	...
	var d = b.getAttribute("tmn_security");
...
scheduler.form_blocks = {
	template: {
		render: function(a) {
			...
			return "<div class='tmn_cal_ltext tmn_cal_template' style='height:" + b + ";'></div>"
		},
	...
	textarea: {
		render: function(a) {
			...
			return "<div class='tmn_cal_ltext' style='height:" + b + ";'><textarea></textarea></div>"
		},
		set_value: function(a, b) {
	...
	select: {
		render: function(a) {
			for (var b = (a.height || "23") + "px", c = "<div class='tmn_cal_ltext' style='height:" + b + ";'><select style='width:100%;'>", d = 0; d < a.options.length; d++) c += "<option value='" + a.options[d].key + "'>" + a.options[d].label + "</option>";
			c += "</select></div>";
			return c
		},
		set_value: function(a, b, c, d) {
			var e = a.firstChild;
			if (!e._tmn_onchange && d.onchange) e.onchange = d.onchange, e._tmn_onchange = !0;
	...
	time: {
		render: function(a) {
			...
			return "<div style='height:30px;padding-top:0px;font-size:inherit;' class='tmn_section_time'>" + g + "<span style='font-weight:normal; font-size:10pt;'> &nbsp;&ndash;&nbsp; </span>" + g + "</div>"
		},
		set_value: function(a, b, c, d) {
			function e(a, b, c) {
			...
			if (f.full_day) {
				if (!a._full_day) {
					var k = "<label class='tmn_fullday'><input type='checkbox' name='full_day' value='true'> " + scheduler.locale.labels.full_day + "&nbsp;</label></input>";
...
scheduler.show_cover = function() {
	...
	this._cover.className = "tmn_cal_cover";
	...
	this.getLightbox().onclick = function(a) {
		...
		if (b && b.className) switch (b.className) {
			...
			case "tmn_cancel_btn":
				...
				break;
			default:
				if (b.getAttribute("tmn_button")) scheduler.callEvent("onLightboxButton", [b.className, b, a]);
				else {
					var d, e, f;
					if (b.className.indexOf("tmn_custom_button") != -1) b.className.indexOf("tmn_custom_button_") != -1 ? (d = b.parentNode.getAttribute("index"), f = b.parentNode.parentNode) : (d = b.getAttribute("index"), f = b.parentNode, b = b.firstChild);
...
scheduler._move_while_dnd = function(a) {
	if (scheduler._dnd_start_lb) {
		if (!document.tmn_unselectable) document.body.className += " tmn_unselectable", document.tmn_unselectable = !0;
...
scheduler._finish_dnd = function() {
	if (scheduler._lb_start) scheduler._lb_start = scheduler._dnd_start_lb = !1, document.body.className = document.body.className.replace(" tmn_unselectable", ""), document.tmn_unselectable = !1
};
scheduler.getLightbox = function() {
	if (!this._lightbox) {
		var a = document.createElement("DIV");
		a.className = "tmn_cal_light";
		scheduler.config.wide_form && (a.className += " tmn_cal_light_wide");
		scheduler.form_blocks.recurring && (a.className += " tmn_cal_light_rec");
		/msie|MSIE 6/.test(navigator.userAgent) && (a.className += " tmn_ie6");
		a.style.visibility = "hidden";
		for (var b = this._lightbox_template, c = this.config.buttons_left, d = 0; d < c.length; d++) b += "<div class='tmn_btn_set tmn_left_btn_set " + c[d] + "_set'><div tmn_button='1' class='" +
			c[d] + "'></div><div>" + scheduler.locale.labels[c[d]] + "</div></div>";
		c = this.config.buttons_right;
		for (d = 0; d < c.length; d++) b += "<div class='tmn_btn_set tmn_right_btn_set " + c[d] + "_set' style='float:right;'><div tmn_button='1' class='" + c[d] + "'></div><div>" + scheduler.locale.labels[c[d]] + "</div></div>";
		b += "</div>";
		a.innerHTML = b;
		...
		for (var e = this.config.lightbox.sections, b = "", d = 0; d < e.length; d++) {
			...
			if (f) {
				...
				var g = "";
				e[d].button && (g = "<div class='tmn_custom_button' index='" + d + "'><div class='tmn_custom_button_" + e[d].button + "'></div><div>" + this.locale.labels["button_" + e[d].button] + "</div></div>");
				this.config.wide_form && (b += "<div class='tmn_wrap_section'>");
				b += "<div id='" + e[d].id + "' class='tmn_cal_lsection'>" +
...
scheduler._lightbox_template = "<div class='tmn_cal_ltitle'><span class='tmn_mark'>&nbsp;</span><span class='tmn_time'></span><span class='tmn_title'></span></div><div class='tmn_cal_larea'></div>";
...
scheduler._touch_events = function(a, b, c) {
	function d(a, b, c) {
		if (a && b) {
			...
			if (e > c && (!d || e / d > 3)) a.pageX > b.pageX ? scheduler._click.tmn_cal_next_button() : scheduler._click.tmn_cal_prev_button()
		}
	}
	...
	dhtmlxEvent(this._els.tmn_cal_data[0], "scroll", e);
	dhtmlxEvent(this._els.tmn_cal_data[0], "touchcancel", e);
	dhtmlxEvent(this._els.tmn_cal_data[0], "contextmenu", function(a) {
		if (l) return a && a.preventDefault && a.preventDefault(), (a || event).cancelBubble = !0, !1
	});
	dhtmlxEvent(this._els.tmn_cal_data[0], a[1], function(a) {
		if (!c(a)) {
			i = j = h = !1;
			l = !0;
			...
			if (d) {
				var e = new Date;
				if (!j && !i && e - n < 250) return scheduler._click.tmn_cal_data(d), window.setTimeout(function() {
				...
				!j && !i && scheduler.config.touch_drag && (k = setTimeout(function() {
					i = !0;
					var a = g.target;
					if (a && a.className && a.className.indexOf("tmn_body") != -1) a = a.previousSibling;
	...
	dhtmlxEvent(this._els.tmn_cal_data[0], a[2], function(a) {
		if (!c(a)) {
			i || d(g, h, 200);
	...
	if (!d) {
		...
		scheduler.attachEvent("onTemplatesReady", function() {
			...
			scheduler.templates.month_day = function(c) {
				if (this._mode == "month") {
					var d = a(c);
					c.getDate() == 1 && (d = scheduler.locale.date.month_full[c.getMonth()] + " " + d); + c == +scheduler.date.date_part(new Date) && (d = scheduler.locale.labels.tmn_cal_today_button + " " + d);
					return d
				} else return b.call(this, c)
			};
			if (scheduler.config.fix_tab_position)
				for (var c = scheduler._els.tmn_cal_navline[0].getElementsByTagName("div"),
					...
					if (n) switch (l.style.right = "auto", n) {
						...
						case "month_tab":
							l.style.left = "136px";
							l.className += " tmn_cal_tab_last";
							break;
						default:
							l.style.left = e + "px", l.className += " tmn_cal_tab_standalone", e = e + 14 + l.offsetWidth
					}
				}
		});
		scheduler._skin_init = function() {}
...
window.jQuery && function(a) {
	var b = [];
	a.fn.tmn_scheduler = function(c) {
		if (typeof c === "string")
			if (b[c]) return b[c].apply(this, []);
			else a.error("Method " + c + " does not exist on jQuery.tmn_scheduler");
			else {
				var d = [];
				this.each(function() {
					if (this && this.getAttribute && !this.getAttribute("tmnscheduler")) {
						for (var a in c) a != "data" && (scheduler.config[a] = c[a]);
						if (!this.getElementsByTagName("div").length) this.innerHTML = '<div class="tmn_cal_navline"><div class="tmn_cal_prev_button">&nbsp;</div><div class="tmn_cal_next_button">&nbsp;</div><div class="tmn_cal_today_button"></div><div class="tmn_cal_date"></div><div class="tmn_cal_tab" name="day_tab" style="right:204px;"></div><div class="tmn_cal_tab" name="week_tab" style="right:140px;"></div><div class="tmn_cal_tab" name="month_tab" style="right:76px;"></div></div><div class="tmn_cal_header"></div><div class="tmn_cal_data"></div>',
						this.className += " tmn_cal_container";
